...
export const parseReplay = (id: string) => {
	...
	const lobbyInfo = {
		...
		id,
		timestamp: matchDates.find((x) => x.id === id)?.timestamp || Math.floor(Date.now() / 1000)
	};
	...
	for (let i = 0; i < rounds.length; i++) {
		const roundDamageEvents = damageEvents.filter(
			(x) => x.tick >= rounds[i].start && x.tick <= rounds[i].end
		);
		rounds[i] = {
			...rounds[i],
			teamOneScore: parseTicks(filePath, ['team_rounds_total'], [rounds[i].start]).find(
				(x) => x.steamid === captainOne.steamid
			).team_rounds_total,
			teamTwoScore: parseTicks(filePath, ['team_rounds_total'], [rounds[i].start]).find(
				(x) => x.steamid === captainTwo.steamid
			).team_rounds_total,
			damage: playerInfo.map((attacker) => {
				const damageDealt = roundDamageEvents.filter(
					(x) => x.attacker_steamid === attacker.steamid
				);
				const enemyTeam = playerInfo.filter((x) => x.team_number !== attacker.team_number);

				const damageToEachPlayer = enemyTeam.map((defender) => {
					const damageDealtToDefender = damageDealt.filter(
						(x) => x.user_steamid === defender.steamid
					);
